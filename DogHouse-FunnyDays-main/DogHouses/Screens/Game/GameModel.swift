...
final class GameModel: ObservableObject {
    ...
    enum Constant {
        ...
        static let cellContents = ["dog1", "collar", "dog3", "coin", "home"]
    ...
    @MainActor func exchange(_ sourceIndex: Int, with destinationIndex: Int) {
    ...
    @MainActor private static func matches(for board: [Cell]) -> Set<Int> {
        Set(
    ...
    static func with<Value>(_ value: Value, update: (inout Value) throws -> Void) rethrows -> Value {
        var copy = value
        try update(&copy)
        return copy
    }
